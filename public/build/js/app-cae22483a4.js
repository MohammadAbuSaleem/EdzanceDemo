	        ...
	        value: function acceptStudent(status, student, index) {
	            ...
	            this.API.all('class/accept/' + this.rootScope.vc.id).post(data).then(function (response) {
	                if (response.errors == true) {
	                    ...
	                    _this.toastr.success(response.data.message);
	                }
	                if (status == 1) {
	                    _this.scope.waitingStudent.splice(index, 1);
	                    _this.rootScope.vc.vcuser.push(student);
	                } else if (status == 0) {
	                    _this.scope.waitingStudent.splice(index, 1);
	                }
	                if (_this.scope.waitingStudent.length == 0) {
	                    _this.scope.noWaitingStudent = true;
	                }
	            });
	        }
	    }, {
