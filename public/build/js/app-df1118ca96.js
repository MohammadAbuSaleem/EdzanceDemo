...
/***/ function(module, exports) {
	...
	var VirtualclassAttendanceController = function () {
	    VirtualclassAttendanceController.$inject = ["$scope", "$filter", "$http", "$q", "API", "$stateParams", "$state", "DTOptionsBuilder", "DTColumnDefBuilder"];
	    function VirtualclassAttendanceController($scope, $filter, $http, $q, API, $stateParams, $state, DTOptionsBuilder, DTColumnDefBuilder) {
	        'ngInject';
	...
	var instituteAboutController = function () {
	  ...
	  function instituteAboutController($scope, $log, $timeout, $uibModal, API, $rootScope) {
	    ...
	    this.$rootScope.newBox = {
	      ...
	      "icon": "info",
	      "data": [{
	        "boxTitle": "معلومات عامه",
	        "boxType": "text",
	        "boxValue": "معلومات عامه معلومات عامه"
	      }]
	    };
	    $scope.about = [{
	      ...
	      this.addNewBox = function (form) {
	        if (form.$valid) {
	          if (_this.classEnabled == true) {
	            _this.classEnabled = false;
	            var mynewclass = {
	              name: _this.rootScope.newPost.name,
	              description: _this.rootScope.newPost.description,
	              permission: _this.rootScope.newPost.permission,
	              tags: _this.rootScope.newPost.type,
	              auth: _this.rootScope.newPost.auth
	            };
	            _this.rootScope.newPost.name = '';
	            _this.rootScope.newPost.description = '';
	            _this.rootScope.newPost.permission = '';
	            _this.rootScope.newPost.type = '';
	            _this.rootScope.newPost.auth = '';
	            _this.cancel();
	            _this.API.all('gr/addgroup').post(mynewclass).then(function (response) {
	              self.rootScope.megroups.push(response.data);
	              // self.scope.$parent.megroups.push(response.data);
	              self.classEnabled = true;
	            }, function (response) {
