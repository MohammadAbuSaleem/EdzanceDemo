...
class mUserController extends Controller
{
    ...
    {
        $user = Auth::user();
        //$users = User::where('id', $user->id)->with('myclass')->with('friends')->get();
        $users = User::where('users.id', $user->id)->with('student_info')->with('instructor_info')->with('myclass')->with('friends')->with('followers')
                       ->join('specialition as spec',  'users.specialition_id', '=','spec.id')
                       ->join('university as uni',  'users.university_id', '=','uni.id')
                        ->select('users.*','spec.mthr_name as spec_name' ,'spec.id as spec_id','uni.mthr_name as uni_name' ,'uni.id as uni_id')->first();
        // $users->id = $id;
