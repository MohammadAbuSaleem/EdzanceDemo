...
class mvcController extends Controller {
    ...
    public function getPost($id, $skip = 0, $take = 10) {
        ...
        $post = classpost::where('class_id', $id)->with('comment')
                        ...
                        ->orderBy('created_at', 'desc')->take($take)->skip($skip)->get();
        foreach ($post as $k => $v) {
            $present = "no exp";
    ...
    public function postHomeWork(Request $request) {
        ...
        //return $request; 
        //$timestamp = strtotime($request->hw_date);
    ...
    public function getHomeWork($id, $skip = 0, $take = 10) {
        $user = Auth::user();
        $mypost = homework::where('virtualclass_id', $id)->with('comment')->orderBy('created_at', 'asc')->take($take)->skip($skip)->get();
    ...
    public function getExam($id, $skip = 0, $take = 10) {
        $user = Auth::user();
        $mypost = exam::where('virtualclass_id', $id)->with('comment')->orderBy('created_at', 'asc')->take($take)->skip($skip)->get();
        return response()->success($mypost);
    }

    public function getFiles($id, $skip = 0, $take = 10) {
        $user = Auth::user();
        $mypost = class_files::where('class_id', $id)->with('comment')->orderBy('created_at', 'asc')->take($take)->skip($skip)->get();
        return response()->success($mypost);
    }
