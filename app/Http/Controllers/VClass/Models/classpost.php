...
class classpost extends Model
{
    ...
    {
        return $this->belongsTo(__NAMESPACE__ . '\virtualclass', 'class_id', 'id');
    }
    ...
    public function PostTag()
    {
        return $this->hasManyThrough(
            __NAMESPACE__ . '\classpost', 'App\Models\tags',
            'tag', 'tags', 'tags'
        );
    }
    ...
    public function scopeIsfriend($query)
    {
        ...
        return $query->leftjoin('friends', function ($join) {
            $user = Auth::user();
            $join->on('friends.friend_id', '=', 'classpost.users_id')
    ...
    public function getPostOwnerToken($vcPostId, $userId)
    {
        ...
        $tokens = $this::select("user_token.token", "classpost.users_id as user_id")
            ->leftJoin("user_token", "user_token.user_id", "=", "classpost.users_id")
            ->where("classpost.id", "=", $vcPostId)
            ->where("classpost.users_id", "!=", $userId)
