...
class User extends Model implements AuthenticatableContract, CanResetPasswordContract, HasRoleAndPermissionContract {
    ...
    public function instructorInfo() {
        return $this->hasOne('App\Models\instructor_info', 'ins_id', 'id');
    }
    ...
    public function myclass() {
        return $this->hasMany('App\Models\classusers', 'uid', 'id')
            ->leftjoin('virtualclass as vc', 'classusers.class_id', '=', 'vc.id')->where('vc.status', 'Activated')->leftjoin('users as ins', 'ins.id', '=', 'vc.instructor')
            ->select('vc.id',
                'vc.status',
                'vc.name',
    ...
    public function friendsinvers() {
        return $this->hasMany('App\Models\friends', 'friend_id', 'id')
            ->leftjoin('users', 'friends.friend_id', '=', 'users.id')
            ->select( /*'users.*'*/'users.id', 'friends.friend_id', 'friends.id', 'friends.status')->where('friends.status', 'follower');
        // ->select('users.*','friends.friend_id','friends.id','friends.status')->where('friends.status','follower')->with('post');
    }

    public function requested() {
        return $this->hasMany('App\Models\request', 'requested_id', 'id')->where('requests.operation', 'follow')->where('requests.place', 'friendship')->where('requests.action', null);
    }
    public function requester() //friendship

    {
        return $this->hasMany('App\Models\request', 'requester_id', 'id')->where('requests.operation', 'follow')->where('requests.place', 'friendship')->where('requests.action', null);
    }
